 ...
 var slowAES = {
    ...
    aes: {
        ...
        core: function (word, iteration) {
            ...
            for (var i = 0; i < 4; ++i) word[i] = this.sbox[word[i]];
        ...
        expandKey: function (key, size) {
            ...
            var expandedKey = [];
            for (var i = 0; i < expandedKeySize; i++) expandedKey[i] = 0;

            /* set the 16,24,32 bytes of the expanded key to the input key */
            for (var j = 0; j < size; j++) expandedKey[j] = key[j];
            currentSize += size;
            ...
            while (currentSize < expandedKeySize) {
                 ...
                 */
                if (currentSize % size == 0) t = this.core(t, rconIteration++);

                /* For 256-bit keys, we add an extra sbox to the calculation */
                if (size == this.keySize.SIZE_256 && currentSize % size == 16)
                    for (var l = 0; l < 4; l++) t[l] = this.sbox[t[l]];
        ...
        createRoundKey: function (expandedKey, roundKeyPointer) {
            ...
            for (var i = 0; i < 4; i++)
                for (var j = 0; j < 4; j++)
                    roundKey[j * 4 + i] =
                        expandedKey[roundKeyPointer + i * 4 + j];
            return roundKey;
        },
        ...
        mixColumns: function (state, isInv) {
            ...
            for (var i = 0; i < 4; i++) {
                ...
                for (var j = 0; j < 4; j++) column[j] = state[j * 4 + i];
                /* apply the mixColumn on one column */
                column = this.mixColumn(column, isInv);
                /* put the values back into the state */
                for (var k = 0; k < 4; k++) state[k * 4 + i] = column[k];
            }
            return state;
        },
        ...
        main: function (state, expandedKey, nbrRounds) {
            state = this.addRoundKey(
                state,
                this.createRoundKey(expandedKey, 0)
            );
            for (var i = 1; i < nbrRounds; i++)
                state = this.round(
                    state,
                    this.createRoundKey(expandedKey, 16 * i)
                );
            state = this.subBytes(state, false);
            state = this.shiftRows(state, false);
            state = this.addRoundKey(
                state,
                this.createRoundKey(expandedKey, 16 * nbrRounds)
            );
            return state;
        },
        ...
        encrypt: function (input, key, size) {
            ...
            for (
                var k = 0;
                k < 4;
                k++ /* unmap the block again into the output */
            )
    ...
    encrypt: function (bytesIn, mode, key, iv) {
        ...
        // char firstRound
        var firstRound = true;
        if (mode == this.modeOfOperation.CBC) this.padBytesIn(bytesIn);
        if (bytesIn !== null) {
            for (var j = 0; j < Math.ceil(bytesIn.length / 16); j++) {
                var start = j * 16;
                var end = j * 16 + 16;
                if (j * 16 + 16 > bytesIn.length) end = bytesIn.length;
                byteArray = this.getBlock(bytesIn, start, end, mode);
                if (mode == this.modeOfOperation.CFB) {
                    if (firstRound) {
                        output = this.aes.encrypt(iv, key, size);
                        firstRound = false;
                    } else output = this.aes.encrypt(input, key, size);
                    for (var i = 0; i < 16; i++)
                    ...
                    for (var i = 0; i < 16; i++)
                        input[i] =
                            byteArray[i] ^ (firstRound ? iv[i] : ciphertext[i]);
                    firstRound = false;
                    ciphertext = this.aes.encrypt(input, key, size);
                    // always 16 bytes because of the padding for CBC
                    for (var k = 0; k < 16; k++) cipherOut.push(ciphertext[k]);
                }
            }
        }
        return cipherOut;
    ...
    decrypt: function (cipherIn, mode, key, iv) {
        ...
        if (cipherIn !== null) {
            for (var j = 0; j < Math.ceil(cipherIn.length / 16); j++) {
                var start = j * 16;
                var end = j * 16 + 16;
                if (j * 16 + 16 > cipherIn.length) end = cipherIn.length;
                ...
                if (mode == this.modeOfOperation.CFB) {
                    ...
                    for (i = 0; i < 16; i++)
                        byteArray[i] =
                            (firstRound ? iv[i] : input[i]) ^ output[i];
                    firstRound = false;
                    for (var k = 0; k < end - start; k++)
                        bytesOut.push(byteArray[k]);
                    input = ciphertext;
            ...
            if (mode == this.modeOfOperation.CBC) this.unpadBytesOut(bytesOut);
        }
        return bytesOut;
    },
    ...
    unpadBytesOut: function (data) {
        ...
        if (data.length > 16) {
            for (
                var i = data.length - 1;
                i >= data.length - 1 - blockSize;
                i--
            ) {
                if (data[i] <= blockSize) {
                    if (padByte == -1) padByte = data[i];
                    if (data[i] != padByte) {
                        padCount = 0;
                        break;
                    }
                ...
                if (padCount == padByte) break;
            }
            if (padCount > 0) data.splice(data.length - padCount, padCount);
        }
    },
    /*
...
module.exports = { slowAES, toNumbers, toHex };
